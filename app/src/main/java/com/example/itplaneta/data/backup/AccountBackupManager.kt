    ...
    override suspend fun backupToUri(uri: Uri): BackupResult<BackupMessage> =
        withContext(Dispatchers.IO) {
            try {
                ...
                Result.Success(
                    BackupMessage(
                        resId = R.plurals.backup_saved_successfully, arg = accounts.size
                    )
                )
            } catch (e: FileNotFoundException) {
                Timber.e(e, "File not found for backup")
                Result.Error(
                    e, BackupMessage(resId = R.string.backup_error_file_not_found)
                )
            } catch (e: IOException) {
                Timber.e(e, "Error saving backup")
                Result.Error(
                    e, BackupMessage(resId = R.string.backup_error_io_save)
                )
            } catch (e: Exception) {
                Timber.e(e, "Unexpected error during backup")
                Result.Error(
                    e, BackupMessage(resId = R.string.backup_error_unexpected)
                )
            }
        }
    ...
    override suspend fun restoreFromUri(uri: Uri): BackupResult<BackupMessage> =
        withContext(Dispatchers.IO) {
            try {
                ...
                try {
                    accounts = backupRepository.deserializeAccounts(text)
                } catch (e: Exception) {
                    Timber.e(e, "Error deserialize json from old backup")
                }

                try {
                    val json = backupRepository.decryptBackup(text)
                    accounts = backupRepository.deserializeAccounts(json)
                } catch (e: Exception) {
                    Timber.e(e, "Error deserialize from new backup")
                }
                ...
                Result.Success(
                    BackupMessage(
                        resId = R.plurals.backup_restored_successfully, arg = accounts.size
                    )
                )
            } catch (e: FileNotFoundException) {
                ...
                Result.Error(
                    e, BackupMessage(resId = R.string.backup_error_io_read)
                )
            } catch (e: IllegalArgumentException) {
                Timber.e(e, "Backup file is empty or invalid")
                Result.Error(
                    e, BackupMessage(resId = R.string.backup_error_empty)
                )
            } catch (e: Exception) {
