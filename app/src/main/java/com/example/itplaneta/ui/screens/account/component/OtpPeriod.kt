...
@Composable
fun OtpPeriod(viewModel: AccountViewModel, onAccountValueChange: (RawAccount) -> Unit) {
    ...
    Column {
        OutlinedTextField(
            ...
            onValueChange = {
                onAccountValueChange(account.copy(period = it))
            },
            isError = state.value.errorType == ErrorType.PeriodError,
            maxLines = 1,
            label = {
                Text(
        ...
        if (state.value.errorType == ErrorType.PeriodError) {
            Text(
